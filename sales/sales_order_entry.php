...
function handle_update_item()
{
	if ($_POST['UpdateItem'] != '' && check_item_data()) {
		$_SESSION['Items']->update_cart_item($_POST['LineNo'],
		 ...
		 input_num('Disc') / 100, $_POST['item_description'],input_num('gst'),input_num('gst_amt'),input_num('cst'),input_num('cst_amt'),input_num('ist'),input_num('ist_amt'),$_POST['hsn_no'],$_POST['currency']);
	}
	page_modified();
  line_start_focus();
}
...
{
//display_error($_POST['hsn_no']);
	if (!check_item_data()) {
			return;
	}
	add_to_order($_SESSION['Items'], get_post('stock_id'), input_num('qty'),
		input_num('price'), input_num('Disc') / 100, get_post('stock_id_text'),input_num('gst'),input_num('gst_amt'),input_num('cst'),input_num('cst_amt'),input_num('ist'),input_num('ist_amt'),$_POST['hsn_no'],$_POST['currency']);
