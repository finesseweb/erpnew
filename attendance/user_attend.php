<?php
$page_security =  'SA_UPDATEATTENDANCEUSER';
if (!@$_GET['popup'])
	$path_to_root = "..";
else	
	$path_to_root = "../..";
...
if (!@$_GET['popup'])
{
	...
	page(("Attendance Entry"), @$_GET['popup'], false, "", $js);
}
...
//function clear_data(){	unset($_POST);	}
        ...
        if (db_has_employees()) {
            ...
            check_cells(_("Show Manually Attendance:"), 'manually_val', null, true);
            ...
            if ($selected_id == 0){
                ...
                while ($row = db_fetch_assoc($selected_empl)) {
                    ...
                    $holiday_sql = "SELECT * FROM fa_user_attendance WHERE a_in_time LIKE ".db_escape("%$date%")." AND empl_id = ".db_escape($row['empl_id']);
                }
             
                    $attendance_res = db_query($holiday_sql);
                   ...
                   // $holiday_sql = "SELECT * FROM fa_user_attendance WHERE a_in_time LIKE ".db_escape("%$date%")." AND empl_id = ".db_escape($row['empl_id']);
                    ...
                    $get_hours = "SELECT half_day FROM fa_user_workinghours";
                    $hours_res = db_query($get_hours);
                    $hours_result23 = db_fetch($hours_res);
                    $given_hours=$hours_result23[0];
                    $date=date('Y-m-d',strtotime($_POST['attendance_date']));
                    $current_date=date('Y-m-d');
                    if($current_date != $date){
                     
                        $holiday_sql12 = "SELECT SUM(working_hours) FROM ".TB_PREF."user_attendance WHERE empl_id=". db_escape($row['empl_id'])." AND a_in_time LIKE". db_escape("%$date%")."AND a_out_time IS NOT NULL";

                        $attendance_res = db_query($holiday_sql12);
                        $attendance_result23 = db_fetch_row($attendance_res);
                        $userWorkhours=round($attendance_result23[0]/3600);
                        $user_hours=$userWorkhours;

                        if ($user_hours < $given_hours && $user_hours !=0) {
                            echo kv_radio(" ", 'Empl_' . $row['empl_id'], "HD", "selected", false, $disabled);
                        }else {
                            echo kv_radio(" ", 'Empl_' . $row['empl_id'], "HD", "", false, $disabled);
                        }
                    }
                    else {
                       echo kv_radio(" ", 'Empl_' . $row['empl_id'], "HD", "", false, $disabled);
