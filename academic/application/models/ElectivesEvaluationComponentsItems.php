 ...
 */
class Application_Model_ElectivesEvaluationComponentsItems extends Zend_Db_Table_Abstract
{
    public $_name = 'electives_evaluation_components_items';
    protected $_id = 'el_eci_id';
...
//print_r($result); die;					  
	...
	public function  getWeightage($component_id){
		$select = $this->_db->select()
				->from($this->_name)
				->where("$this->_name.el_eci_id=?",$component_id);
		$result = $this->getAdapter()
					->fetchRow($select);
		return $result;			
